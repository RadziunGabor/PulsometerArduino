...
class MesaurementPuls(QDialog):
    ...
    def savedata(self):
        ...
        iduser = nowid
        cur = conn.cursor()
        query4 = "Insert into dbo.Pomiar(id_użytkownika, value, data) values('" + str(
            iduser) + "', '" + measurevalue + "', '" + todaydate + "')"
        cur.execute(query4)
        conn.commit()

        QMessageBox.warning(self, 'Komunikat',
                            'Pomiar o wartości: ' + measurevalue + ' BPM w dniu: ' + todaydate + ' dodano poprawnie.')
...
class Data(QDialog):
    ...
    def downloaddata(self):
        ...
        if count != 0:
            query2 = "select value from dbo.Pomiar where id_użytkownika = '" + str(
                nowid) + "' and data = '" + choosedata + "'"
            value = cur.execute(query2).fetchall()

            for row in value:
                self.listWidget.addItem(str(row[0]) + " BPM")
